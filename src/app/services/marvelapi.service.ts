...
export class MarvelapiService {
  ...
  getCharacters(offset = 0): Observable<CharacterDataWrapper> {
    return this.http.get<CharacterDataWrapper>(
      `${environment.apiUrl}characters?apikey=${environment.apiKey}&offset=${offset}&hash=${this.hash}&ts=${this.ts}`
    );
  }

  getCharacterDetails(id:any): Observable<CharacterDataWrapper> {
    return this.http.get<CharacterDataWrapper>(
      `${environment.apiUrl}characters/${id}?apikey=${environment.apiKey}&hash=${this.hash}&ts=${this.ts}`
    );
  }

  getComics(offset = 0): Observable<ComicDataWrapper> {
    return this.http.get<ComicDataWrapper>(
      `${environment.apiUrl}comics?&apikey=${environment.apiKey}&offset=${offset}&orderBy=-issueNumber&limit=3&hash=${this.hash}&ts=${this.ts}`
    );
  }

  getComicDetails(id:number): Observable<ComicDataContainer> {
    return this.http.get<ComicDataContainer>(
      `${environment.apiUrl}comics/${id}?&apikey=${environment.apiKey}&hash=${this.hash}&ts=${this.ts}`
    );
  }

  getCreators(offset = 0): Observable<CreatorDataWrapper> {
    return this.http.get<CreatorDataWrapper>(
      `${environment.apiUrl}creators?&apikey=${environment.apiKey}&offset=${offset}&hash=${this.hash}&ts=${this.ts}`
    );
  }

  getCreatorDetails(id:number): Observable<CreatorDataContainer> {
    return this.http.get<CreatorDataContainer>(
      `${environment.apiUrl}creators/${id}?&apikey=${environment.apiKey}&hash=${this.hash}&ts=${this.ts}`
    );
  }
